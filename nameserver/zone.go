...
// The zone database uses some other mechanisms (eg, mDNS) for finding unknown information
// or for keeping current information updated.
type zoneDb struct {
	mx     sync.RWMutex
...
func NewZoneDb(config ZoneConfig) (zone *zoneDb, err error) {
	...
	if zone.mdnsSrv == nil {
		if zone.mdnsSrv, err = NewMDNSServer(zone); err != nil {
			return
		}
	}
...
func (zone *zoneDb) Start() (err error) {
	...
	if zone.refreshInterval > 0 {
		Info.Printf("[zonedb] Starting %d background updaters", defaultNumUpdaters)
...
// Obatin the domain where this zone works on
